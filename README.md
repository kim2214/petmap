...
- `app/src/main/assets/petmap.db` — 한국문화정보원 "전국 반려동물 동반 가능 문화시설" + "문화 여가 활동 시설(액티비티)" 데이터셋을 정제해 넣은 SQLite DB(v3). `이름+좌표` 기준 중복 제거 후 **동반 가능(Y) 장소만 약 22,127개**를 수록한다(동반 불가 N 은 제외).
